\documentclass[a4paper, 12pt]{report}

\usepackage[italian]{babel}
\usepackage[utf8]{inputenc}

% Chapters are now called problems.
\addto\captionsitalian{
    \renewcommand\chaptername{Problema}}

\usepackage[table]{xcolor}
\usepackage{tabularx}

\usepackage{xspace}
\usepackage{enumerate}
\usepackage{epigraph}
\usepackage{fullpage}

\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amsfonts}

\usepackage{tikz}
\usetikzlibrary{shapes.geometric,calc,backgrounds}

\usepackage{algorithm}
%\usepackage{algorithmicx}
\usepackage[noend]{algpseudocode}

% Hacks needed to get the algorithm package to work.
\renewcommand{\algorithmiccomment}[1]{/\(\!\)/ #1}
\floatname{algorithm}{Algoritmo}

\theoremstyle{plain}
\newtheorem{definition}{Definizione}
\newtheorem*{definition*}{Definizione}
\newtheorem{problem}{Problema}
\newtheorem*{problem*}{Problema}
\newtheorem{lemma}{Lemma}
\newtheorem*{proof*}{Dimostrazione}

\newcommand{\quicksort}{\textsc{Quicksort}\xspace}
\newcommand{\distributionsort}{\textsc{Distribution Sort}\xspace}
\newcommand{\mergesort}{\textsc{Merge Sort}\xspace}
\newcommand{\vectornorm}[1]{\left|\left|#1\right|\right|}

% Command used in 01.tex
\newcommand{\kwaymerge}{\textsc{\(k\)-way Merge}\xspace}
\newcommand{\minheap}{\textsc{Min Heap}\xspace}

% Command used in 03.tex
\newcommand{\sort}{\mbox{sort}}

% Styles used in 04.tex
\tikzstyle{el}=[circle,draw=black!100,fill]
\tikzstyle{red_el}=[circle,draw=red!100,fill=red!100]
\tikzstyle{green_el}=[circle,draw=green!100,fill=green!100]

% Commands used in 05.tex
\newcommand{\map}{\textsc{Map}\xspace}
\newcommand{\reduce}{\textsc{Reduce}\xspace}
\newcommand{\mapreduce}{\textsc{MapReduce}\xspace}
\newcommand{\parser}{\textsc{Parser}\xspace}

% Commands used in 08.tex
\newcommand{\dc}{\textsc{Dc3}\xspace}
\newcommand{\radixsort}{\textsc{Radix Sort}\xspace}

% Command used in 18.tex
\newcommand{\LZuno}{\textsc{Lz77}\xspace}

% Command used in 19.tex
\newcommand{\LZdue}{\textsc{Lz78}\xspace}

% Commands and styles used in 21.tex
\newcommand{\maxsat}{\textsc{Max-Sat}\xspace}
\tikzstyle{literal}=[circle,draw=black!100,fill,inner sep=0pt,minimum size=0.1cm]

% Command used in 22.tex
\newcommand{\E}{\mathbb{E}}

\begin{document}
    \title{Problemi di Algoritmica 2}
    \author{
        Alessandro Ambrosano\\
        \texttt{alessandro.ambrosano [at] gmail.com}
        \and
        Jacopo Notarstefano\\
        \texttt{jacopo.notarstefano [at] gmail.com}
        \and
        Lorenzo Pezzella\\
        \texttt{lorenzo.pezzella [at] gmail.com}
        \and
        Francesco Salvatori\\
        \texttt{gabbianogiallo [at] alice.it}
    }
    
    \maketitle

    \input{quote.tex}
    \input{01.tex}
    \input{02.tex}
    \input{03.tex} 
    \input{04.tex}
    \input{05.tex}
    \input{06.tex}
    \input{07.tex}
    \input{08.tex}
    \input{09.tex}
    \input{10.tex}
    \input{11.tex}
    \input{12.tex}
    \input{13.tex}
    \input{14.tex}
    \input{15.tex}
    \input{16.tex}
    \input{17.tex}
    \input{18.tex}
    \input{19.tex}
    \input{20.tex}
    \input{21.tex}
    \input{22.tex}
\end{document}
